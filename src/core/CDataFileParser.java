...
public class CDataFileParser {
	...
	private static String LOGS_FOLDER_NAME = "data2";
	
	private static String name;
	private static String session;
	private static long timeTotal = 0;
	private static long inactivityTimeTotal = 0;
		
	private static List<CEvent> mouseEventsList = new ArrayList<>();
	private static List<CEvent> keyboardEventsList = new ArrayList<>();
	
	private static List<Long> pauseTimeList = new ArrayList<>();
	private static List<Long> windowSwitchedTimeList = new ArrayList<>();
	
	private static CKeyEvent keyEvent = new CKeyEvent();
	private static HashMap<String, CKeyAction> pressedKeysMap = new HashMap<>();
	
	private static int neutral=0, positive=0, negative=0;
	private static boolean emotionSwitched = false;
	
	public static void start() throws IOException
	{
		File logsFolder = new File(LOGS_FOLDER_NAME);
		File[] peopleFiles = logsFolder.listFiles();
		
		for(int p=0; p<peopleFiles.length; p++)
		{
			name = peopleFiles[p].getName();
			
			//if(name.equals("test")){
				openPeopleFiles(peopleFiles[p].listFiles());
			//}
	...
	public static void openPeopleFiles(File[] sessions)
	{
		for(int s=0; s<sessions.length; s++)
		{
			session = sessions[s].getName();
			
			//currentEmotion = null;
			openSession(sessions[s].listFiles());
			if(mouseEventsList.size() > 0)
			{
	...
	public static void openSession(File[] logFiles)
	{		
		CMousePair<CMyPoint, CMyPoint> startPointPair = new CMousePair<CMyPoint, CMyPoint>(null, null);
		CMousePair<CMyPoint, CMyPoint> clicksPair = new CMousePair<CMyPoint, CMyPoint>(null, null);
		
		boolean mouseActivity = false;
		boolean clicked = false;		
		long currentTimeStamp = 0;
		long previousTimeStamp = 0;
		EEmotionEnum currentEmotion = null;
		int clickCounter = 0;
		int pauseCounter = 0;
		int switchedWindowCounter = 0;
		...
		for(int l=0; l<logFiles.length; l++)
		{
			...
			try 
			{
				lineList = (ArrayList<String>) Files.readAllLines(Paths.get(f.getPath()));
			} 
			catch (IOException e) {
