...
public class NativeHookDemo extends JFrame implements ActionListener, ItemListener, NativeKeyListener, NativeMouseInputListener, NativeMouseWheelListener, WindowListener {
	...
	static {
		try {
			NativeUtils.loadLibraryFromJar("/JNativeHook.dll"); // during
																// runtime. .DLL
																// within .JAR
	...
	public static CWritter dataCollector;
	
	private static final int MAX_TITLE_LENGTH = 1024;
	
	private String currentFocusedWindow;
	
	private static NativeHookDemo nativeHook;

	
	public static NativeHookDemo getInstance()
	{
		if(nativeHook == null)
		{			
			nativeHook = new NativeHookDemo();
		}
		
		return nativeHook;
	}
	
	public JRadioButton getPositiveRadioButton()
	{
		return positive;
	}
	
	public JRadioButton getNeutralRadioButton()
	{
		return neutral;
	}
	
	public JRadioButton getNegativeRadioButton()
	{
		return negative;
	}
	...
	public NativeHookDemo() {
		...
		dataCollector = CWritter.getInstance();
		...
		positive.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				CWritter.writeText("RADIOBUTTON="+EEmotionEnum.POSITIVE.toString());
				CWritter.getInstance().setEmotion(EEmotionEnum.POSITIVE.toString());
				
				CExperimentTimer.getInstance().resetTimer(CAppConstants.DIALOG_TIMER);				
			}			
		});
		...
		neutral.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				CWritter.getInstance();
				CWritter.writeText("RADIOBUTTON="+EEmotionEnum.NEUTRAL.toString());
		...
		negative.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				CWritter.getInstance();
				CWritter.writeText("RADIOBUTTON="+EEmotionEnum.NEGATIVE.toString());
